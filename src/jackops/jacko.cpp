...
struct JackoState {
    ...
    std::atomic<bool> jacko_has_finished;
    ...
    JackoState(CSOUND *csound_, const char *serverName_, const char *clientName_)
        : csound(csound_), serverName(serverName_), clientName(clientName_),
          jacko_is_driving(false), /*jack_active(false),*/ jacko_has_finished(true) {
        int result = 0;
        csound = csound_;
        ...
        if (!jackClient) {
            ...
            csound->Message(
                csound, "Created Jack client \"%s\" for Jack server \"%s\". Status: %d\n",
                clientName, serverName, status);
        }
        jackFramesPerTick = jack_get_buffer_size(jackClient);
        if (csoundFramesPerTick != jackFramesPerTick) {
            csound->Message(
                csound, "Jack buffer size %d != Csound ksmps %d, exiting...\n",
        ...
        if (csoundFramesPerSecond != jackFramesPerSecond) {
            csound->Message(
                csound, "Jack sampling rate %d != Csound sr %d, exiting...\n",
...
// to sleep when it comes here!
        if (jacko_is_driving.load() == false) {
...
// until the Csound performance is complete.
            csound->Message(csound,
                            "%s", "Jacko is now driving Csound performance...\n");
            ...
            lock.unlock();
            csound->Message(csound,
                            "%s", "Jacko has quit driving Csound performance.\n");
            return 1;
        }
        return result;
    }
...
// the Jack connection in a separate thread.
            if (finished /* && jack_active */) {
                ...
                jacko_is_driving = false;
                csound->Message(csound, "%s", "Csound performance driven by Jack has finished.\n");
// Create a thread to run the close routine.
    ...
    int close() {
        jacko_has_finished = true;
        csound->Message(csound, "%s", "JackoState::close...\n");
        jacko_is_driving = false;
        int result = OK;
// Try not to do thread related operations more than once...
        result = jack_deactivate(jackClient);
        csound->Message(csound, "%s", "Jack client deactivated.\n");
// Jack documentation says deactivating the client does the following also:
        ...
        csound->Message(csound, "%s", "Jack ports unregistered.\n");
        result |= jack_client_close(jackClient);
        csound->Message(csound, "%s", "Jack client closed.\n");
        std::unique_lock lock(csoundPerformanceThreadConditionMutex);
    ...
    static void *closeThreadRoutine_(void *userdata) {
        void *result = ((JackoState *)userdata)->closeThreadRoutine();
        return result;
    }
    void startTransport() {
...
static int midiRead_(CSOUND *csound, void *userData, unsigned char *midiData,
    ...
    if (jackoState_->jacko_has_finished == true) {
        return 0;
    }
    int midiI = 0;
...
struct JackoFreewheel : public OpcodeBase<JackoFreewheel> {
    ...
    int init(CSOUND *csound) {
        ...
        if (result) {
            warn(csound,
                 "Failed to set Jack freewheeling mode to \"%s\": error %d.\n",
...
struct JackoOn : public OpcodeBase<JackoOn> {
    ...
    int init(CSOUND *csound) {
        ...
        JackoState *jackoState = 0;
        csound::QueryGlobalPointer(csound, "jackoState", jackoState);
        log(csound, "JackoOn is deprecated and now is a no-op.\n");
        return result;
    }
};
...
// State.
    ...
    int init(CSOUND *csound) {
        ...
        if (csoundPort) {
            log(csound, "Found Csound audio in port \"%s\".\n", csoundFullPortName);
        } else {
            csoundPort =
                jack_port_register(jackoState->jackClient, csoundPortName,
                                   JACK_DEFAULT_AUDIO_TYPE, JackPortIsInput, 0);
            if (csoundPort) {
                log(csound, "Created Csound audio in port \"%s\".\n", csoundFullPortName);
            } else {
                warn(csound, "Could not create Csound audio in port \"%s\".\n",
                     csoundFullPortName);
            }
        }
        externalPortName = csound->strarg2name(
                               csound, (char *)0, SexternalPortName->data, (char *)"csound", (int)1);
        result = jack_connect(jackoState->jackClient, externalPortName,
                              csoundFullPortName);
        if (result == EEXIST) {
            log(csound, "Jack audio connection from \"%s\" to \"%s\" already exists.\n",
                csoundFullPortName, externalPortName);
        } else if (result) {
            warn(csound,
                 "Could not create Jack audio connection from \"%s\" to \"%s\": "
                 "status %d %s.\n",
                 externalPortName, csoundFullPortName, errno, std::strerror(errno));
            return result;
        } else {
            log(csound, "Created Jack audio connection from \"%s\" to \"%s\".\n",
...
// State.
    ...
    int audio(CSOUND *csound) {
        IGN(csound);
        if (jackoState->jacko_has_finished == true) {
            return OK;
        }
...
// State.
    ...
    int init(CSOUND *csound) {
        ...
        if (!csoundPort) {
            ...
            if (csoundPort) {
                log(csound, "Created Csound audio out port \"%s\".\n", csoundFullPortName);
            } else {
                warn(csound, "Could not create Csound audio out port \"%s\".\n",
                     csoundFullPortName);
            }
        }
        ...
        if (result == EEXIST) {
            ...
            warn(csound,
                 "Could not create Jack connection from \"%s\" to \"%s\": "
                 "status %d %s.\n",
                 csoundFullPortName, externalPortName, errno, std::strerror(errno));
            return result;
        } else {
...
// State.
    ...
    int init(CSOUND *csound) {
        ...
        csoundPort = jack_port_by_name(jackoState->jackClient, csoundFullPortName);
        if (csoundPort) {
            log(csound, "Found Csound MIDI in port \"%s\".\n", csoundFullPortName);
        } else {
            csoundPort =
                jack_port_register(jackoState->jackClient, csoundPortName,
                                   JACK_DEFAULT_MIDI_TYPE, JackPortIsInput, 0);
            if (csoundPort) {
                log(csound, "Created Csound MIDI in port \"%s\".\n", csoundFullPortName);
            } else {
                warn(csound, "Could not create Csound MIDI in port \"%s\".\n",
                     csoundFullPortName);
            }
        }
        externalPortName = csound->strarg2name(
                               csound, (char *)0, SexternalPortName->data, (char *)"csound", (int)1);
        result = jack_connect(jackoState->jackClient, externalPortName, csoundFullPortName);
        if (result == EEXIST) {
            log(csound, "Jack MIDI connection from \"%s\" to \"%s\" already exists.\n",
                externalPortName, csoundFullPortName);
        } else if (result) {
            warn(csound,
                 "Could not create Jack MIDI connection from \"%s\" to \"%s\": "
                 "status %d.\n",
...
// State.
    ...
    int init(CSOUND *csound) {
        ...
        csoundPort = jack_port_by_name(jackoState->jackClient, csoundFullPortName);
        if (csoundPort) {
            log(csound, "Found Csound MIDI out port \"%s\".\n", csoundFullPortName);
        } else {
            csoundPort =
                jack_port_register(jackoState->jackClient, csoundPortName,
                                   JACK_DEFAULT_MIDI_TYPE, JackPortIsOutput, 0);
            if (csoundPort) {
                log(csound, "Created Csound MIDI out port \"%s\".\n", csoundFullPortName);
            } else {
                warn(csound, "Could not create Csound out MIDI port \"%s\".\n",
                     csoundFullPortName);
            }
        }
        externalPortName = csound->strarg2name(
                               csound, (char *)0, SexternalPortName->data, (char *)"csound", (int)1);
        result = jack_connect(jackoState->jackClient, csoundFullPortName,
                              externalPortName);
        if (result == EEXIST) {
            log(csound, "Jack MIDI connection from \"%s\" to \"%s\" already exists.\n",
                csoundFullPortName, externalPortName);
...
extern "C" {
    ...
    PUBLIC int csoundModuleDestroy(CSOUND *csound) {
        ...
        csound::QueryGlobalPointer(csound, "jackoState", jackoState);
        if (jackoState) {
            if (jackoState->jacko_has_finished == false) {
                jackoState->close();
            }
            delete jackoState;
            jackoState = 0;
